...
{
    public partial class inhoadon1 : Form
    {
        public inhoadon1()
        {
            InitializeComponent();
        ...
        public inhoadon1(long maDH) : this()
        {
            maDonHang = maDH;
            LoadDonHang();
        }
                 ...
                 WHERE dh.MaDonHang = @MaDonHang";
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                label5.Text = reader["MaDonHang"].ToString();
                                label4.Text = reader["MaNhanVien"].ToString();

                                string hoTen = reader["Ho"].ToString() + " " + reader["Ten"].ToString();
                                label11.Text = hoTen;

                                label12.Text = reader["SoDienThoai"].ToString();
                                label13.Text = reader["DiaChi"].ToString();

                                label14.Text = Convert.ToDateTime(reader["NgayDatHang"]).ToString("dd/MM/yyyy");
                                label18.Text = Convert.ToDecimal(reader["TongTien"]).ToString("N0") + " VND";
                                label20.Text = Convert.ToDecimal(reader["TongTien"]).ToString("N0") + " VND";
                            }
                        }
                    }
